...
/**
 * ChatList component that displays a scrollable list of chat conversations.
 * Supports filtering by search term and shows chat names, last message previews, and timestamps.
 * Handles chat selection and displays appropriate avatars for group vs private chats.
 * @param chats - Array of chat objects to display
 * @param selectedChatId - ID of the currently selected chat (for highlighting)
 * @param onSelectChat - Callback function when a chat is clicked
 * @param user - Current authenticated user object
 * @param search - Search term to filter chats by name
 */
export const ChatList: React.FC<ChatListProps> = ({ chats, selectedChatId, onSelectChat, user, search }) => {

    /**
     * Determines the display name for a chat based on its type and participants.
     * For group chats, returns the group name. For private chats, returns the other participant's name.
     * @param chat - The chat object to get the name for
    ...
    // Filter chats based on search term matching chat names
